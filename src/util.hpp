 ...
 * that you can define the binary functions for multiple integer types without
 * redefinition errors. Call this macro in the body of a struct or class like
 * so: struct mybitfield { int value; MAKE_INTEGER_FUNCS_BINARY(mybitfield,
 * value, int)
 *  };
 * @see MAKE_INTEGER_FUNCS_UNARY().
 * @param cname Class name these function definitions will return.
 * @param membername The member of `cname` that will be used for integer
 * operations.
